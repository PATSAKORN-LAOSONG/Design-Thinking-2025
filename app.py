import streamlit as st
import pandas as pd

st.set_page_config(page_title="ระบบประเมินความดัน", layout="centered")

st.title("🩺 ระบบประเมินระดับความดันโลหิต")
st.write("กรอกค่าความดันโลหิต ระบบจะประเมินระดับตามข้อมูลจากไฟล์ CSV")

# =========================
# โหลดไฟล์ CSV
# =========================
@st.cache_data
def load_data():
    df = pd.read_csv("table_1.csv")
    df.columns = df.columns.str.strip()  # ลบช่องว่างหัวคอลัมน์
    return df

df = load_data()

# =========================
# ตรวจสอบว่าไฟล์มีข้อมูล
# =========================
if df.empty:
    st.error("ไฟล์ CSV ไม่มีข้อมูล")
    st.stop()

# ใช้คอลัมน์แรกเป็นชื่อระดับ
category_col = df.columns[0]

# =========================
# รับค่าผู้ใช้
# =========================
sys = st.number_input("ค่า SYSTOLIC (ตัวบน)", 0, 300, 120)
dia = st.number_input("ค่า DIASTOLIC (ตัวล่าง)", 0, 200, 80)

# =========================
# ตรวจสอบระดับ
# =========================
if st.button("🔍 ตรวจสอบระดับความดัน"):

    st.subheader("📊 ผลการประเมิน")

    # 1️⃣ คำนวณระดับตามเกณฑ์มาตรฐาน
    if sys >= 180 or dia >= 120:
        level = 5
    elif sys >= 140 or dia >= 90:
        level = 4
    elif sys >= 130 or dia >= 80:
        level = 3
    elif sys >= 120 and dia < 80:
        level = 2
    else:
        level = 1

    # 2️⃣ ดึงชื่อระดับจากไฟล์ CSV
    df_sorted = df.reset_index(drop=True)

    if level <= len(df_sorted):
        result = str(df_sorted.iloc[level - 1][category_col]).strip()
    else:
        result = "ไม่พบระดับ"

    # 3️⃣ แสดงผล
# 3️⃣ แสดงผลภาษาอังกฤษ (จาก Excel)
st.success(f"ระดับความดันภาษาอังกฤษของคุณคือ: **{result}**")

result_en = result  # เก็บค่าจาก Excel ไว้

# 4️⃣ แปลเป็นภาษาไทย
if level == 5:
    result_th = "ภาวะวิกฤตความดันโลหิตสูง"
    st.error("⚠️ ระดับอันตรายมาก ควรพบแพทย์ทันที")

elif level == 4:
    result_th = "ความดันโลหิตสูง ระดับที่ 2"
    st.error("⚠️ ความดันสูงระดับ 2 ควรปรึกษาแพทย์")

elif level == 3:
    result_th = "ความดันโลหิตสูง ระดับที่ 1"
    st.warning("ควรควบคุมอาหารและติดตามอาการ")

elif level == 2:
    result_th = "ความดันสูงกว่าปกติเล็กน้อย"
    st.info("ความดันเริ่มสูง ควรปรับพฤติกรรม")

else:
    result_th = "ปกติ"
    st.success("อยู่ในระดับปกติ")

# แสดงผลภาษาไทย
st.write(f"ระดับความดันภาษาไทยของคุณคือ: **{result_th}**")
